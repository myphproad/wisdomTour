...
function seo($title = '', $keywords = '', $description = '',$footer = '')
{
    if (!empty($title)) {
...
function get_cover_url($cover_id, $width = '', $height = '') {
  /*  if ($width > 0 && $height > 0) {
    ...
    $map ['status'] = 1;
    $map ['id'] = $cover_id;
    $picture = db ( 'carousel' )->where ( $map )->find();
 ...
 * @author huajie <banhuajie@163.com>
 */
function get_carousel_title($cover_id){
    $map ['status'] = 1;
    $picture = db ( 'carousel' )->where ( $map )->getById ( $cover_id );
...
/*取背景颜色的值*/
function get_bgcolor_title($cover_id){
    $map ['id'] = $cover_id;
    $bgcolor = db ( 'index_bgstyle' )->where ( $map )->find();
