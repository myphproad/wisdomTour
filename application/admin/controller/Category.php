...
class Category extends Common
{
    ...
    public function insert()
    {
        if (request()->isPost()) {
            ...
            if(!empty($data['carousel_id'])){
                $data['carousel_id']= implode(',', $data['carousel_id']);
            }
            $catid = db('category')->insertGetId($data);
            if ($catid > 0) {
                $url = __ROOT__ . '/index.php/index/lists.html?catid=' . $catid;
    ...
    public function edit()
    {
        ...
        $index_bgstyle_result = db('index_bgstyle')->select();

        $index_site_id = db('index_site')->select();

        $carousel_id = db('carousel')->select();

        $this->assign('list', $list);
        $this->assign('detail', $detail);
        $this->assign('index_bgstyle_result', $index_bgstyle_result);
        $this->assign('index_site_id', $index_site_id);
        $this->assign('carousel_id', $carousel_id);//幻灯片分类
